...
In this guide you will learn how you can map, import, load and filter data on TRIADB. There are existing data models and data resources that have been created during the testing of TRIADB modules. See the `installation guide <https://github.com/healiseu/triadb/blob/master/INSTALLATION.rst>`_ for more details.
...
---------
    ...
    For more details visit the file ``SPC_ASERD.py`` which demonstrates `breadth-first-search` (BFS) used in filtering ASETs. For example if we start filtering with an attribute of `PRT` entity, it will propagate filtering through ``p_id`` attribute to `CAT` entity and then through ``s_id`` to `SUP` entity. That is the subject of the following tutorial exercise.
...
-------------------------
    ...
    Selections are written in *Chain Query Language* (CQL) with a functional and OOP style to specify programmatically operations on `HACOL` sets. It is super easy and intuitive to learn and at the same time it saves your time from writing complex queries. For example:
...
This second tutorial uses the classic example of Northwind Traders relational database.
...
The data model is already built during the rebuild-population procedure (see Installation Guide). Now start Jupyter Lab.
...
Northwind Dashboard
-------------------
In the last part of our tutorial you will see an example of a dashboard web application built with TriaClick and `Dash <https://dash.plot.ly/>`__ . It uses associative selections to interact with the various graphical elements. Open a shell and enter the following commands
...
That concludes our tutorial guide, there is also a screen capture demo of the Northwind dashboard that you can watch `here <https://www.youtube.com/watch?v=QSk1ldfb7ow>`_
